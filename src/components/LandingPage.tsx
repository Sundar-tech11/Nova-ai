...
export default function LandingPage({ onLaunchWorkspace }: { onLaunchWorkspace?: (tab: string) => void }) {
  const { login, signup, loginWithGoogle, loginWithGithub, forgotPassword, resetPasswordSubmit, isAuthenticated, error: authError } = useAuth();

  // Modal Control States
  const [isAuthModalOpen, setIsAuthModalOpen] = useState(() => {
    return sessionStorage.getItem('ns_recovery_mode') === 'reset';
  });
  const [authMode, setAuthMode] = useState<'login' | 'signup' | 'forgot' | 'reset'>(() => {
    return sessionStorage.getItem('ns_recovery_mode') === 'reset' ? 'reset' : 'login';
  });
  
  // Login Form Configuration
  const [email, setEmail] = useState(() => {
    return sessionStorage.getItem('ns_recovery_email') || '';
  });
  ...
  const [simulatedCode, setSimulatedCode] = useState(() => {
    return sessionStorage.getItem('ns_recovery_code') || '';
  });
  ...
  const handleForgotSubmit = async (e: FormEvent) => {
    ...
    if (result.success) {
      const code = result.resetCode || '992147';
      setSimulatedCode(code);
      sessionStorage.setItem('ns_recovery_mode', 'reset');
      sessionStorage.setItem('ns_recovery_email', email);
      sessionStorage.setItem('ns_recovery_code', code);
  ...
  const handleResetSubmit = async (e: FormEvent) => {
    ...
    if (result.success) {
      ...
      sessionStorage.removeItem('ns_recovery_mode');
      sessionStorage.removeItem('ns_recovery_email');
      sessionStorage.removeItem('ns_recovery_code');
      setTimeout(() => {
        setAuthMode('login');
        setPassword('');
        clearMessages();
  ...
  const handleSocialLogin = async (provider: 'Google' | 'GitHub') => {
    ...
    try {
      ...
      if (result.success) {
        setLocalSuccess(`Successfully authenticated via ${provider}! Initializing workstation...`);
        setTimeout(() => {
          setIsAuthModalOpen(false);
          window.history.pushState({}, '', '/dashboard');
          window.dispatchEvent(new Event('popstate'));
        ...
        setLocalError(result.error || `${provider} authentication was not completed.`);
      }
    } catch (err: any) {
      setLocalError(err.message || `${provider} SSO pipeline failure.`);
    } finally {
      setFormLoading(false);
    }
  };
        ...
        {isAuthModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            >
              ...
              {localError && (
                <div className={`mb-4 p-4 rounded-xl border text-2xs flex flex-col gap-3 text-left animate-in fade-in ${
                  localError.includes('unauthorized-domain') || localError.includes('not authorized for this domain')
                    ? 'border-amber-500/30 bg-amber-500/5 text-amber-200' 
                    : 'border-red-500/20 bg-red-500/5 text-[#ea7878]'
                }`}>
                  <div className="flex items-start gap-2">
                    ...
                    <span className="whitespace-pre-line leading-relaxed">{localError}</span>
                  </div>
                  ...
                  {(localError.includes('unauthorized-domain') || localError.includes('not authorized for this domain')) && (
                    <div className="flex flex-wrap gap-2 pt-2 border-t border-amber-500/10">
                      <button
                        ...
                        className="px-2.5 py-1 rounded bg-amber-500/10 hover:bg-amber-500/20 text-[10px] font-mono font-medium text-amber-300 transition-all flex items-center gap-1 cursor-pointer"
                      >
                        {copied ? 'Copied Hostname!' : `Copy '${window.location.hostname}'`}
                      </button>
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        className="px-2.5 py-1 rounded bg-indigo-500/15 hover:bg-indigo-500/25 text-[10px] font-medium text-indigo-300 transition-all cursor-pointer"
                      >
