...
export default function SkillsTracker() {
  ...
  // Track if local state has modifications that are pending sync
  const [isSkillsDirty, setIsSkillsDirty] = useState(false);
  const [isGoalsDirty, setIsGoalsDirty] = useState(false);

  // Sync from authenticated user on mount / user change, only if NOT local dirty
  useEffect(() => {
    if (user) {
      if (!isSkillsDirty) {
  ...
  }, [user, isSkillsDirty, isGoalsDirty]);

  // Debounced skills sync
  useEffect(() => {
    if (!isSkillsDirty) return;
    
    const timer = setTimeout(async () => {
      if (user) {
        try {
          await updateProfile({ skillsProgress: skills });
          setIsSkillsDirty(false);
        } catch (err) {
          console.error("Failed to sync skills progress:", err);
        }
      }
    }, 800);

    return () => clearTimeout(timer);
  }, [skills, isSkillsDirty, user]);

  // Debounced goals sync
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      if (user) {
        try {
          await updateProfile({ learningGoals: goals });
          setIsGoalsDirty(false);
        } catch (err) {
          console.error("Failed to sync learning goals:", err);
        }
      }
    }, 800);

    return () => clearTimeout(timer);
  }, [goals, isGoalsDirty, user]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {skills.length === 0 ? (
              <div className="col-span-full py-12 text-center border border-dashed border-slate-800 bg-slate-900/10 rounded-2xl space-y-4">
                <p className="text-xs text-slate-400">No tracked skills yet.</p>
                <button
                  type="button"
                  onClick={() => setShowAddForm(true)}
                  className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs cursor-pointer transition-colors"
                >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900/30 border border-slate-800 p-6 rounded-2xl space-y-4 relative overflow-hidden">
            ...
            <div className="space-y-3 pt-2">
              {goals.length === 0 ? (
                <div className="py-10 text-center border border-dashed border-slate-800 bg-slate-950/20 rounded-xl space-y-3">
                  <p className="text-2xs text-slate-400">No learning goals yet.</p>
                  <button
                    type="button"
                    onClick={() => setShowGoalForm(true)}
                    className="px-3 py-1.5 rounded-lg bg-pink-600 hover:bg-pink-500 text-white font-bold text-3xs cursor-pointer transition-colors"
                  >
                    + Create Goal
                  </button>
                </div>
