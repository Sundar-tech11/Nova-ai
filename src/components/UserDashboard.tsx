...
export default function UserDashboard({ onNavigate }: UserDashboardProps) {
  const { user, token, getDashboardSummary, isLoading: authLoading } = useAuth();
  ...
  const isFirstVisit = !user?.onboardingCompleted && 
  ...
  // Recommendation 1: Resume Scorer
  if (stats.resumesCount === 0) {
    dynamicRecommendations.push({
      title: "Analyze your resume",
      subtitle: "Upload your resume in PDF/TXT format to get a complete ATS score and keyword gap analysis.",
      actionLabel: "Analyze Resume",
      path: "/resume"
    });
  } else if (stats.avgResumeScore < 80) {
    dynamicRecommendations.push({
      title: "Improve ATS match indexes",
      subtitle: `Your average resume score is ${stats.avgResumeScore}%. Incorporate quantitative STAR bullet points.`,
      actionLabel: "Optimize Resume",
      path: "/resume"
    });
  } else {
    dynamicRecommendations.push({
      title: "Elite resume status achieved!",
      subtitle: "Your resume stands within the elite tier. Focus on polishing showcase systems.",
      actionLabel: "View Resume Audits",
      path: "/resume"
    });
  }

  // Recommendation 2: Roadmaps
  if (stats.roadmapsCount === 0) {
    dynamicRecommendations.push({
      title: "Synthesize a career roadmap",
      subtitle: "Define a target role and custom background to build a comprehensive studying timeline.",
      actionLabel: "Create Roadmap",
      path: "/roadmaps"
    });
  } else {
    dynamicRecommendations.push({
      title: "Advance roadmap stages",
      subtitle: "Check off your phase elements and build custom portfolio showcase items.",
      actionLabel: "View Roadmaps",
      path: "/roadmaps"
    });
  }

  // Recommendation 3: Interview Prep
  if (stats.interviewsCount === 0) {
    dynamicRecommendations.push({
      title: "Practice a mock interview",
      subtitle: "Simulate a high-density behavior/tech screen scored directly on STAR metrics.",
      actionLabel: "Practice Interview",
      path: "/interview"
    });
  } else if (stats.avgInterviewScore < 85) {
    dynamicRecommendations.push({
      title: "Polishing STAR response depth",
      subtitle: `Your average interview score is ${stats.avgInterviewScore}%. Try doing another panel run.`,
      actionLabel: "Simulate Sandbox",
      path: "/interview"
    });
  } else {
    dynamicRecommendations.push({
      title: "Highly ready for live rounds!",
      subtitle: "Excellent adherence to STAR structure. Maintain your focus on complex scenarios.",
      actionLabel: "Interrogate Sandbox",
      path: "/interview"
    });
  }

  // Recommendation 4: Skills ledger
  const trackedSkillsCount = user?.skillsProgress?.length || stats.skillsCount || 0;
  if (trackedSkillsCount < 3) {
    dynamicRecommendations.push({
      title: "Track your technical competencies",
      subtitle: "Add your active skills in the Skills Ledger to record progress metrics.",
      actionLabel: "Skills Ledger",
      path: "/skills"
    });
  } else {
    dynamicRecommendations.push({
      title: "Log your learning objectives",
      subtitle: "Formulate next-action targets and claim completion badges.",
      actionLabel: "Log Objectives",
      path: "/skills"
    });
  }
    ...
    >
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          <div className="space-y-3">
            ...
            <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-white leading-none">
              {isFirstVisit ? 'Welcome, ' : 'Welcome back, '}<span className="bg-clip-text text-transparent bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-400 font-black">{displayName}</span>!
            </h1>
            ...
            <div className="pt-2 max-w-md space-y-2 select-none">
              <div className="flex items-center justify-between text-3xs font-mono">
                ...
                {stats.resumesCount > 0 && stats.avgResumeScore > 0 ? (
              ...
              {stats.resumesCount > 0 && stats.avgResumeScore > 0 ? (
                <>
                  <div className="w-full h-2 rounded-full bg-slate-950 overflow-hidden relative border border-white/[0.04]">
                    <motion.div 
                      initial={{ width: 0 }}
                      animate={{ width: `${stats.avgResumeScore}%` }}
                ...
                <p className="text-xs text-slate-400 bg-slate-950/60 p-3 rounded-xl border border-dashed border-slate-900/60 font-light leading-relaxed">
          ...
          <div className="flex gap-4 items-center border border-slate-800/80 bg-slate-950/60 rounded-2xl p-4 shrink-0 sm:self-start md:self-auto shadow-lg hover:border-indigo-500/20 transition-all duration-300">
            ...
            <div>
              ...
              <button 
                ...
                className="text-[10px] font-bold text-indigo-400 hover:text-indigo-300 flex items-center gap-1 mt-1.5 py-1.5 pr-4 transition-colors min-h-[44px] cursor-pointer"
               >
      ...
      >
        <div className="rounded-2xl border border-white/[0.04] bg-slate-950/45 p-5 relative overflow-hidden shadow-sm hover:border-indigo-500/10 hover:shadow-[0_0_15px_rgba(99,102,241,0.05)] transition-all duration-300">
          ...
          <span className="text-[10px] text-slate-400 mt-2 block font-light">Resume analyzer entries count</span>
        </div>
        ...
        <div className="rounded-2xl border border-white/[0.04] bg-slate-950/45 p-5 relative overflow-hidden shadow-sm hover:border-indigo-500/10 hover:shadow-[0_0_15px_rgba(99,102,241,0.05)] transition-all duration-300">
          ...
          <span className="text-[10px] text-slate-400 mt-2 block font-light">Career guided roadmaps tracks</span>
        </div>
        ...
        <div className="rounded-2xl border border-white/[0.04] bg-slate-950/45 p-5 relative overflow-hidden shadow-sm hover:border-indigo-500/10 hover:shadow-[0_0_15px_rgba(99,102,241,0.05)] transition-all duration-300">
          ...
          <span className="text-[10px] text-slate-400 mt-2 block font-light">Simulated voice/chat panel runs</span>
        </div>
            ...
            {recentActivity.length === 0 ? (
              <div className="py-12 text-center text-slate-400 space-y-3">
                ...
                <button
                  ...
                  className="px-4 py-2.5 text-3xs font-bold bg-indigo-500/10 text-indigo-400 rounded-lg hover:bg-indigo-500/20 border border-indigo-500/15 min-h-[44px] flex items-center justify-center mx-auto transition-all cursor-pointer"
                >
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-900 pb-5">
          ...
          <button 
            ...
            className="px-4 py-3 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl text-2xs flex items-center justify-center gap-1 shrink-0 self-start sm:self-center transition-colors cursor-pointer min-h-[44px]"
          >
          ...
          {dynamicRecommendations.map((item, idx) => (
            ...
            >
              ...
              <button 
                ...
                className="text-[10px] font-bold text-blue-400 hover:text-blue-300 transition-colors flex items-center gap-1 mt-4 py-2 pr-4 cursor-pointer self-start min-h-[44px]"
              >
