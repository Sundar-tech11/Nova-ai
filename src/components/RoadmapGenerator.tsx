...
export default function RoadmapGenerator() {
  ...
  const handleGenerate = async (careerName?: string) => {
    ...
    try {
      ...
      let fallbackRoadmap: RoadmapResult = {
        careerGoal: targetGoal,
      ...
      // Specific fallback tweaks based on selected career for extreme realism
      if (targetGoal.includes('AIML')) {
        fallbackRoadmap.requiredSkills = ['Python & PyTorch', 'Linear Algebra / Stats', 'Scikit-Learn', 'Hugging Face API', 'Vector Databases (Chroma/Pinecone)', 'MLOps / MLflow'];
        fallbackRoadmap.timeline[0].topics = ['Python NumPy state, loops, and vectorization arrays', 'Calculus, differentiation, and matrices fundamentals', 'Statistical distributions and descriptive metrics'];
        fallbackRoadmap.timeline[1].topics = ['Supervised algorithms: Linear, Logistics, Decision Trees', 'Neural networks with PyTorch: Backpropagation & Optimization', 'Vector spaces and embeddings setup'];
        fallbackRoadmap.timeline[2].topics = ['Transformers, attention layers, and Hugging Face pipelines', 'Building REST endpoints serving inference models', 'Fine-tuning small models with custom data assets'];
        fallbackRoadmap.projects = [
          {
        ...
        fallbackRoadmap.requiredSkills = ['Linux Administration', 'Network Protocols (TCP/IP)', 'OWASP Top 10 Audit', 'Wireshark Análisis', 'SIEM Logs / Splunk', 'Metasploit Sandbox'];
        fallbackRoadmap.timeline[0].topics = ['Bash scripting, file permissions, and process management', 'Network subnetting, routing tables, and DNS setups', 'Port mapping and scanning fundamentals (Nmap)'];
        fallbackRoadmap.projects = [
          {
