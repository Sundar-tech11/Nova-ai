...
export default function AuthPages({ onSuccess, defaultMode = 'login' }: AuthPagesProps) {
  const { login, signup, loginWithGoogle, loginWithGithub, forgotPassword, resetPasswordSubmit, error: authError } = useAuth();
  
  const [mode, setMode] = useState<'login' | 'signup' | 'forgot' | 'reset'>(() => {
    return (sessionStorage.getItem('ns_recovery_mode') as any) || defaultMode;
  });
  ...
  // Google/GitHub Integrated Real Authentication
  const handleSocialLogin = async (provider: 'Google' | 'GitHub') => {
    ...
    try {
      ...
      if (result.success) {
        setLocalSuccess(`Successfully authenticated via ${provider}! Initializing workspace...`);
  ...
  const handleForgotSubmit = async (e: FormEvent) => {
    ...
    if (result.success) {
      const code = result.resetCode || '123456';
      ...
      <div className="bg-slate-950/90 rounded-xl px-6 py-8 sm:px-8 border border-slate-800/80">
        ...
        {localError && (
          <motion.div 
            ...
            className={`mb-5 p-4 rounded-xl border text-xs flex flex-col gap-3 ${
