...
export default function App() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {isAuthenticated && user ? (
              <div className="relative">
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2.5 min-h-[44px] rounded-xl border transition-all hover:bg-slate-900/40 cursor-pointer select-none ${
            ...
            <button
              ...
              className="lg:hidden p-3 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-xl text-slate-400 hover:text-slate-200 cursor-pointer"
      ...
      {isMobileMenuOpen && (
        <div className={`lg:hidden fixed inset-x-0 top-[73px] bottom-0 z-40 transition-all border-b shadow-2xl ${
          isDarkMode ? 'bg-slate-950/95 border-slate-900 shadow-black' : 'bg-slate-50/95 border-slate-200 shadow-slate-200'
        }`}>
          <div className="p-4 space-y-2 select-none max-h-full overflow-y-auto">
            {navItems.map((item) => {
              const Icon = item.icon;
              const isActive = currentPath === item.path;
              return (
                <button
                  ...
                  className={`w-full px-4 py-3.5 rounded-xl font-bold text-xs text-left transition-all flex items-center gap-3.5 cursor-pointer min-h-[44px] ${
                    isActive
                      ? isDarkMode 
            ...
            {!isAuthenticated && (
              <div className="grid grid-cols-2 gap-2 pt-4 border-t border-slate-900/40 mt-4 pb-6">
                <button
                  onClick={() => navigateTo('/login')}
                  className={`px-4 py-3.5 min-h-[44px] flex items-center justify-center font-bold text-xs border rounded-xl text-center transition-colors cursor-pointer ${
                ...
                <button
                  onClick={() => navigateTo('/signup')}
                  className="px-4 py-3.5 min-h-[44px] flex items-center justify-center font-bold text-xs bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-center transition-colors cursor-pointer"
                >
                  Join Pro
                </button>
              </div>
