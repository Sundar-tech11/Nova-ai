...
// GitHub Sign-In Single Sign On
app.post('/api/auth/github-sso', async (req, res) => {
  try {
    const { uid, name, email, avatar } = req.body;
    const user = await db.getOrCreateGithubUser(uid, name || 'GitHub User', email || null, avatar || '');
    const session = await db.createSession(user.id);
    return res.json({
      token: session.token,
      user: {
    ...
    return res.status(500).json({ error: 'Internal system error during GitHub SSO authentication.', details: err.message });
  }
});
