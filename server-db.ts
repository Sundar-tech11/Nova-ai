...
export async function getOrCreateGithubUser(uid: string, name: string | null, email: string | null, avatarUrl: string): Promise<User> {
  const emailToUse = email ? email.toLowerCase() : `github-${uid}@placeholder.com`;
...
export async function addResumeAnalysis(userId: string, resumeName: string, score: number, atsScore: number, resultData: any): Promise<ResumeAnalysis> {
  ...
  try {
    ...
    console.error('Failed to update onboarding completed status', err);
  }
  return newAnalysis;
}
